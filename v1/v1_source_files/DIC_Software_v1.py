...
class DIC_app(ttk.Frame):
    ...
    def outputs_screen(self):
        ...
        self.ax.set_xlabel('Horizontal displacement [mm]')
        self.ax.set_ylabel('Load [N]')
        self.output_data = []
        self.load_data = []
        self.time_data = []
        self.plotted_data, = self.ax.plot(self.load_data,self.output_data,'kx-',label='Tracked area 1')
    ...
    def run_analysis(self):
        ...
        for i in range(1,len(self.flnms)):
            ...
            self.output_data = self.output_data+[[it for lst in self.track_areas_co_ords for it in lst]]
            ...
            for j in range(len(self.track_areas_co_ords)):
                ...
                # self.track_areas_co_ords[j] = np.array(self.track_areas_co_ords[j])+np.array([corr_max[1],corr_max[0],corr_max[1],corr_max[0]])
        ...
        for ik in range(len(self.track_areas_co_ords)):
            headers = headers+['Horizontal displacement - Tracked area '+str(ik+1)+' [mm]','Vertical displacement - Tracked area '+str(ik+1)+' [mm]']
            output_data = output_data+[list(output_dat[:,(ik*4)]),list(output_dat[:,(ik*4)+1])]
    ...
    def update_plots(self):
        ...
        plt_data = plt_data-plt_data[0]
        try:
            self.plotted_data.set_data(plt_data[:,0],self.load_data[:len(plt_data)])
            self.ax.set_xlim(np.min(plt_data[:,0])-1e-10,np.max(plt_data[:,0]))
