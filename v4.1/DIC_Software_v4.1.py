...
class DIC_app(ttk.Frame):
    ...
    def analysis_parameters(self):
        ...
        self.output_path = r'C:\Users\S1879083\OneDrive - University of Edinburgh\Edi_Python\Video_tracking\Video_tracking_app\v4.1\Output'
        self.output_file = 'Output_data'
        self.calibration_photo_path = r'C:\Users\S1879083\OneDrive - University of Edinburgh\Edi_Python\Video_tracking\Video_tracking_app\v4.1\Calibration_photos'
    ...
    def update_screen(self):
        ...
        if self.screen_number==0:
            ...
            try:
                ...
                self.error_message = 'Output folder does not exist!'
                ...
                self.error_message = 'Calibration files do not exist!'
                if self.remove_image_distortion == True:
                    if os.path.isfile(self.calibration_photo_path+'/Calibration_file_part_1.txt')==False:
    ...
    def browsefunc_calibration_photo_path(self):
        ...
        self.calibration_app.lift()

        try:

            if len(self.calibration_flnms)>0:
                self.calibration_flnms =  glob.glob(self.calibration_photo_path+'\*.jpg')
                self.displayed_calibration_image_menu.destroy()
                self.calibration_button.destroy()
                self.no_calibration_label.destroy()
                self.current_img_label.destroy()
                self.calibration_status_label.destroy()

                self.update_calibration_photo()

        except:
            0
        self.update_based_on_calibration_folder()
    ...
    def update_calibration_photo(self):
        ...
        if len(self.calibration_flnms)>0:
            ...
            try:
                try:
                    self.calibration_data_exists_label.destroy()
                except:
                    pass
                ...
                self.calibration_data_exists_label = ttk.Label(self.corrected_canvas,
                                                      text='No calibration files in\nspecified folder path',
                                                      font=('Helvetica', 15),justify=tk.CENTER)
                self.calibration_data_exists_label.place(relx=0.5,rely=0.5,anchor=tk.CENTER)


        else:
            self.distort_canvas.delete('all')
            self.corrected_canvas.delete('all')
            print('Working')

    def update_based_on_calibration_folder(self):
        try:
            self.no_calibration_label.destroy()
        except:
            pass
        ...
        if len(self.calibration_flnms)>0:
            ...
            self.update_calibration_photo()
            self.current_img_label = ttk.Label(self.displayed_calibration_canvas,text='Current displayed calibration image')
            self.current_img_label.place(relx= .5, rely= .1, anchor= tk.W)
            ...
            self.no_calibration_label = ttk.Label(self.distort_canvas,
                                                  text='No calibration photos in\nspecified folder path',
                                                  font=('Helvetica', 15),justify=tk.CENTER)
            self.no_calibration_label.place(relx=0.5,rely=0.5,anchor=tk.CENTER)
    ...
    def add_distortion_options(self):
        if self.distortion_removal_option==1:
            ...
            self.calibration_file_labelpath.place(relx= .15, rely= .6, anchor= tk.W)
